...
export class LSFWrapper {
  ...
  selectTask(task, annotationID, fromHistory = false) {
   ...
   setLSFProjectXML(project_xml) {
     ...
     // if this is uncommented it will always select masking as the annotation type
     ...
     //  '\t </BrushLabels>\n'+
     //  '</View>');
     // this.lsf.assignConfig(hardcodedxml);
  ...
  prepareData(annotation, { includeId, draft } = {}) {
    ...
    console.log('loaded data ' + annotation.loadedDate);
    console.log('lead time ' + annotation.leadTime);
    const result = {
